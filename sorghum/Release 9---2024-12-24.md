...
The Germplasm tab of the results of a gene query now includes variants that cause the loss of a gene's function (LOF) from 46 ex-PVP breeding lines and Search results are now grouped and tallied by species or clade.
...
- Sorghum BTx623 (NCBIv3)
  ...
  * 36.1 million SNPs from the [Sorghum Genomics Toolbox](https://www.globalsorghuminitiative.org/) initiative called in 940 resequenced genomes using the JGIv5 assembly (see [release notes for version 8.0](https://www.sorghumbase.org/relnotes?section=Release%208)) were lifted to the NCBIv3 assembly. 
...
* The gene search suggestions now include a taxonomy label following the number of genes containing the term. This helps users distinguish similar gene names assigned to multiple species. If a search term is associated with genes from multiple genomes, the lowest common ancestor in the taxonomy tree for these genomes is appended.
...
Standard rsID identifiers assigned by the EVA were mapped to nearly 41 million SNPs in both BTx623 reference assemblies (NCBIv3 and JGIv5.1), as well as all other sorghum genomes including the 46 ex-PVP breeding lines. In addition, rsIDs were also assigned to Arabidopsis, Japonica rice and B73 maize SNPs.
...
Nearly 41 million standard rsIDs from EVA release 5 (July 2023) were mapped from BTx623 (NCBIv3) to 16 sorghum genomes and 46 ex-PVP breeding accessions in SorghumBase.
...
[**Gene Trees**](https://ensembl.sorghumbase.org/prot_tree_stats.html). There are 44,841 protein-coding gene family trees and 12,258 putative split genes for 46 plant genomes in SorghumBase (ex-PVP breeding lines excluded). 
