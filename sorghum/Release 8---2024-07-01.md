...
1) We are providing in this release, the genomes of 46 ex-PVP breeding lines (44 bicolor and 2 drummondii). Ex-PVP accessions are breeding lines with an expired Plant Variety Protection (PVP) Act certificate. These accessions were sequenced and assembled by the groups of Dr. Michael Todd of the Salk Institute and Dr. Nadia Shakoor of the Donald Danforth Plant Science Center. 
  ...
  - _Sorghum bicolor (L.) Moench nothosubsp. drummondii (Steud.) de Wet ex Davidse_: PI564085 (WKMIII) and PI595221 (EXCELS235).


2) A total of 66,928 pan-genes developed at the Cold Spring Harbor Laboratory (CSHL) using annotations from the 28 sorghum genomes in SorghumBase release 7, were lifted to each of the above ex-PVP accessions using the Liftoff tool (Shumate _et al_, 2020). The lifted pan-gene annotations are provided as a track on the corresponding ex-PVP browser pages.
...
- Sorghum BTx623 (JGI_V5.1)
  ...
  1) Approximately 41 million rsIDs from the European Variation Archive (EVA) were mapped to SNPs lifted from the NCBIv3 to the JGI_V5.1 assembly using the [variant remapping pipeline](https://github.com/EBIvariation/variant-remapping/tree/master) at EMBL-EBI. Sample genotypes and their corresponding population allele/genotype frequencies have not been mapped to JGI_V5.1, yet continue to be available in NCBIv3 coordinates from the [Sorghum BTx623 NCBIv3 assembly browser](https://ensembl.sorghumbase.org/Sorghum_bicolor/Info/Index). Non-reference allele (nra) mappings are provided in a separate track on the JGI_V5.1 browser, see for example region XXX.
     
  2) 36.1 million SNPs were called in 940 resequenced genomes from the Gates-funded [Global Sorghum Initiative: Sorghum Genomics Toolbox](https://www.globalsorghuminitiative.org/). This data was provided in JGI_V5.1 genomic coordinates and is the result of an effort led by Dr. Nadia Shakoor at the Donald Danforth Plant Science Center, Dr. John Lovell at the HudsonAlpha Institute for Biotechnology, and Dr. Geoff Morris at Colorado State University.
...
41 million rsIDs were mapped from BTx623 v3.1 to the 46 ex-PVP breeding lines by the SorghumBase Team and are available in their corresponding browsers, see for example rsXXX. 
