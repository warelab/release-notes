...
KAPEEL, ANDREW --> Please describe in one sentence the "Lifoff of the pangenomes as a pangene track" in here.
Sharon need to configure the tracks on the browser. I have provide a one sentence summary below:

A total of 66,928 pan-genes developed at CSHL labs using annotations 28 Sorghum genomes was lifted to each ex-PVP accession using Liftoff tool. The lifted pan-gene annotation is provided as track on the browser pages.

Note: Genes in these assemblies have not been included in gene family trees nor assigned InterPro domains. However WGAs will be made. <-- Is this still the case, JERRY?
No WGAs are being done for this release.
